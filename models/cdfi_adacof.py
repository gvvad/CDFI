...
class CDFI_adacof(torch.nn.Module):
    def __init__(self, args):
        ...
        self.context_synthesis = GridNet(126, 3)  # (in_channel, out_channel) = (126, 3) for the synthesis network
    ...
    def forward(self, frame0, frame2):
        ...
        if w0 % 32 != 0:
            ...
            w_padded = True
        Weight1, Alpha1, Beta1, Weight2, Alpha2, Beta2, Occlusion, Blend, featConv1, featConv2, featConv3, featConv4, featConv5 \
            = self.get_kernel(moduleNormalize(frame0), moduleNormalize(frame2))

        tensorAdaCoF1 = self.moduleAdaCoF(self.modulePad(frame0), Weight1, Alpha1, Beta1, self.dilation) * 1.
        tensorAdaCoF2 = self.moduleAdaCoF(self.modulePad(frame2), Weight2, Alpha2, Beta2, self.dilation) * 1.

        w, h = self.modulePad(frame0).shape[2:]

        tensorConv1_ = F.interpolate(featConv1, size=(w, h), mode='bilinear', align_corners=False)
        tensorConv1L = self.moduleAdaCoF(tensorConv1_, Weight1, Alpha1, Beta1, self.dilation) * 1.
        tensorConv1R = self.moduleAdaCoF(tensorConv1_, Weight2, Alpha2, Beta2, self.dilation) * 1.

        tensorConv2_ = F.interpolate(featConv2, size=(w, h), mode='bilinear', align_corners=False)
        tensorConv2L = self.moduleAdaCoF(tensorConv2_, Weight1, Alpha1, Beta1, self.dilation) * 1.
        tensorConv2R = self.moduleAdaCoF(tensorConv2_, Weight2, Alpha2, Beta2, self.dilation) * 1.

        tensorConv3_ = F.interpolate(featConv3, size=(w, h), mode='bilinear', align_corners=False)
        tensorConv3L = self.moduleAdaCoF(tensorConv3_, Weight1, Alpha1, Beta1, self.dilation) * 1.
        tensorConv3R = self.moduleAdaCoF(tensorConv3_, Weight2, Alpha2, Beta2, self.dilation) * 1.

        tensorConv4_ = F.interpolate(featConv4, size=(w, h), mode='bilinear', align_corners=False)
        tensorConv4L = self.moduleAdaCoF(tensorConv4_, Weight1, Alpha1, Beta1, self.dilation) * 1.
        tensorConv4R = self.moduleAdaCoF(tensorConv4_, Weight2, Alpha2, Beta2, self.dilation) * 1.

        tensorConv5_ = F.interpolate(featConv5, size=(w, h), mode='bilinear', align_corners=False)
        tensorConv5L = self.moduleAdaCoF(tensorConv5_, Weight1, Alpha1, Beta1, self.dilation) * 1.
        tensorConv5R = self.moduleAdaCoF(tensorConv5_, Weight2, Alpha2, Beta2, self.dilation) * 1.

        tensorCombined = torch.cat(
            [tensorAdaCoF1, tensorAdaCoF2, tensorConv1L, tensorConv1R, tensorConv2L, tensorConv2R, tensorConv3L,
             tensorConv3R, tensorConv4L, tensorConv4R, tensorConv5L, tensorConv5R], dim=1)

        frame1_warp = Occlusion * tensorAdaCoF1 + (1 - Occlusion) * tensorAdaCoF2
        frame1_feat = self.context_synthesis(tensorCombined)

        frame1 = Blend * frame1_feat + (1 - Blend) * frame1_warp  # blending of the feature warp and ordinary warp
        ...
        if self.training:
            ...
            g_Beta1 = CharbonnierFunc(m_Beta1[:, :, :, :-1] - m_Beta1[:, :, :, 1:]) + CharbonnierFunc(
                m_Beta1[:, :, :-1, :] - m_Beta1[:, :, 1:, :])
            g_Alpha2 = CharbonnierFunc(m_Alpha2[:, :, :, :-1] - m_Alpha2[:, :, :, 1:]) + CharbonnierFunc(
                m_Alpha2[:, :, :-1, :] - m_Alpha2[:, :, 1:, :])
            g_Beta2 = CharbonnierFunc(m_Beta2[:, :, :, :-1] - m_Beta2[:, :, :, 1:]) + CharbonnierFunc(
                m_Beta2[:, :, :-1, :] - m_Beta2[:, :, 1:, :])

            g_Spatial = g_Alpha1 + g_Beta1 + g_Alpha2 + g_Beta2  # used as total variation loss during training
...
class PrunedKernelEstimation(torch.nn.Module):
    def __init__(self, kernel_size):
        ...
        self.moduleOcclusion = torch.nn.Sequential(
            ...
            torch.nn.Conv2d(in_channels=52, out_channels=52, kernel_size=3, stride=1, padding=1),
            torch.nn.ReLU(inplace=False),
            torch.nn.Conv2d(in_channels=52, out_channels=48, kernel_size=3, stride=1, padding=1),
            torch.nn.ReLU(inplace=False),
            torch.nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True),
    ...
    def forward(self, rfield0, rfield2):
        ...
        return Weight1, Alpha1, Beta1, Weight2, Alpha2, Beta2, Occlusion, Blend, featConv1, featConv2, featConv3, featConv4, featConv5
